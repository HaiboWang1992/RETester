...
def write_header_to_csv(file_name):
    header = ['bug_report_url', 'commit_html_url']
...
def crawl_bug_reports_from_repo(current_line, bug_reports_url_list, repo_name, checkDict):
    bug_report_url = bug_reports_url_list[current_line]
    bug_report_id = str(bug_report_url).strip().split('/')[4]
...
def test_crawl_idea_patch():
    ...
    source_file_path = 'source_file/ideabugreports.csv'
    row_name = 'url'
    ...
    bug_reports_url_list = read_raw_info_from_csv(source_file_path, row_name)[:601]
    total_len = len(bug_reports_url_list)
    ...
    while current_line < total_len:
        error_at = crawl_bug_reports_from_repo(current_line, bug_reports_url_list, repo_name, checkDict)
    ...
    with open(target_file_path, 'a+', encoding='utf-8', newline='') as file:
        ...
        for key in checkDict.keys():
            ...
            one_data_list.append(str(value.get('bug_report_url')))
